...
elseif _have_required_tools(bindir, required) && get(ENV, "BNC_FORCE_REBUILD_CDD", "0") != "1"
    @info "Local cdd backend already available." bindir timestamp=Dates.now(UTC)
else
    try
        run(`bash $script`)
        _have_required_tools(bindir, required) || error("Local cdd build finished but expected tools are missing in $bindir")
        @info "Local cdd backend build completed." bindir timestamp=Dates.now(UTC)
    catch err
        @warn "Local cdd backend build failed; runtime will fall back to NativePolyhedra where needed." exception=(err, catch_backtrace())
    end
end
