...
function transitiveclosure(
    L::SparseMatrixCSC{Tv, Ti}, Jlen, CJ, mark; countonly=false
) where {Tv, Ti <: Integer}
    b = 0
    c = Jlen
    for i in 1:Jlen
...
function solvevec(
    L::SparseMatrixCSC{Tv, Ti}, lowertriangular, x::Vector{Tv}, J, CJ, mark
) where {Tv, Ti <: Integer}
...
function solvemat(
    L::SparseMatrixCSC{Tv, Ti}, B::SparseMatrixCSC{Tv, Ti}; lowertriangular=true
) where {Tv, Ti <: Integer}
    ...
    return SparseMatrixCSC{Tv, Ti}(B.m, B.n, CP, RV, NZ)
end
...
function solve(
    ...
    solvemode=detect,
    numthreads=min(B.n, Threads.nthreads()),
) where {Tv, Ti <: Integer}
    if solvemode == detect
        if istril(L)
            solvemode = lower
    ...
    Threads.@threads for j in 1:numthreads
...
function Base.:\(A::luFac{Tv, Ti}, B::SparseMatrixCSC{Tv, Ti}) where {Tv, Ti <: Integer}
    ...
    if !ismissing(A.L)
        B = solve(A.L, B; solvemode=lower)
    end
    if !ismissing(A.U)
        B = solve(A.U, B; solvemode=upper)
    end
    if !ismissing(A.q)
        B = B[A.q, :]
    end
...
function Base.:\(A::luFac{Tv, Ti}, B::SparseVector{Tv, Ti}) where {Tv, Ti <: Integer}
    return SparseVector(A \ SparseMatrixCSC(B))
end
function Base.:\(
    A::SparseMatrixCSC{Tv, Ti}, B::SparseMatrixCSC{Tv, Ti}
) where {Tv, Ti <: Integer}
    return luFac(A) \ B
end
function Base.:\(
    A::SparseMatrixCSC{Tv, Ti}, B::SparseVector{Tv, Ti}
) where {Tv, Ti <: Integer}
    return luFac(A) \ B
end
function Base.transpose(A::luFac)
    return luFac(
        ismissing(A.U) ? missing : sparse(transpose(A.U)),
        ismissing(A.L) ? missing : sparse(transpose(A.L)),
        ismissing(A.q) ? missing : invperm(A.q),
        ismissing(A.p) ? missing : invperm(A.p),
    )
end
function Base.:/(A::SparseMatrixCSC{Tv, Ti}, B::luFac{Tv, Ti}) where {Tv, Ti <: Integer}
    return sparse(transpose(transpose(B) \ sparse(transpose(A))))
end
function Base.:/(
    A::SparseMatrixCSC{Tv, Ti}, B::SparseMatrixCSC{Tv, Ti}
) where {Tv, Ti <: Integer}
...
function Base.inv(A::SparseMatrixCSC{Tv, Ti}) where {Tv, Ti <: Integer}
    return A \ spdiagm(0 => ones(Tv, size(A, 1)))
end
