function slice_polyhedron(
    poly::Polyhedron; fixed_idx::AbstractVector{<:Integer}, fixed_value::Real=1.0
)::Polyhedron
    n = fulldim(poly)
...
function _grid_sample_polyhedron(poly::Polyhedron, bounds; npoints::Int=10000)
    ...
    return Point3f[Point3f(x, y, z) for x in xs, y in ys, z in zs if [x, y, z] ∈ poly]
end
...
function plot_polyhedron_slices(
    ...
    sliced = [
        slice_polyhedron(p; fixed_idx=fixed_idx, fixed_value=fixed_value) for p in polys
    ]
    labels =
        isnothing(labels) ? ["poly $i" for i in eachindex(polys)] : collect(string.(labels))
    axis_labels =
        isnothing(axis_labels) ? ["dim 1", "dim 2", "dim 3"] : collect(string.(axis_labels))
    colors = isnothing(colors) ? Makie.wong_colors() : collect(colors)

    samples = [
        isempty(p) ? Point3f[] : _grid_sample_polyhedron(p, bounds; npoints=npoints) for
        p in sliced
    ]
    ...
    for (i, pts) in enumerate(samples)
        ...
        scatter!(
            ax,
            pts;
            color=(colors[mod1(i, length(colors))], alpha),
