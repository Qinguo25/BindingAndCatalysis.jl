export find_catalysis_regimes!,
    get_catalysis_network,
    get_catalysis_regime,
    get_catalysis_regimes,
    get_catalysis_regimes_dict
...
function find_catalysis_regimes!(args...; kwargs...)
    return find_catalysis_regimes!(get_catalysis_network(args...; kwargs...))
end
...
@inline function _build_catalysis_regimes(model::CatalysisData, all_regimes, is_asymptotic)
    ...
    for i in 1:n_regimes
        regimes[i] = CatalysisRegime(;
            network=model, perm=all_regimes[i], idx=i, is_asymptotic=is_asymptotic[i]
        )
    end
    return regimes
end
...
get_affine_xk2v(cn::CatalysisData) =
    let
