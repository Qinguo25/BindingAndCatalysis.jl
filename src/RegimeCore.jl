export n_bind_regimes,
    n_catalysis_regimes,
    n_bnc_regimes,
    get_binding_network,
    get_catalysis_network,
    is_feasible
export ensure_binding_regimes!,
    ensure_catalysis_regimes!, ensure_bnc_regimes!, ensure_regime_data!
export get_binding_regime, get_binding_regimes, get_binding_perms, get_binding_indices
export get_catalysis_perms, get_catalysis_indices, get_bnc_perms, get_bnc_indices
...
@inline _bnc_linear_index(n_bind::Int, bind_idx::Int, cat_idx::Int) =
    bind_idx + (cat_idx - 1) * n_bind
@inline _bnc_cart_index(n_bind::Int, idx::Int) =
    ((idx - 1) % n_bind + 1, (idx - 1) ÷ n_bind + 1)
...
function get_catalysis_network(model::Bnc, args...)
    let
        if isnothing(model.catalysis)
            error(
                "Model does not contain a catalysis network. Please provide a Bnc model with a catalysis network.",
            )
        end
...
@inline is_catalysis_regimes_built(model::CatalysisData) =
...
@inline _bind_regimes(model::Bnc) =
    let
        if isnothing(model.BindRegimes)
            error(
                "Model does not contain binding regimes. Please run \"find_all_regimes!($model)\" to compute the binding regimes.",
            )
        end
        return model.BindRegimes
    end
@inline _catalysis_regimes(model::CatalysisData) =
    let
        if isnothing(model.CatalysisRegimes)
            error(
                "Model does not contain catalysis regimes. Please run \"find_catalysis_regimes!($model)\" to compute the catalysis regimes.",
            )
        end
        return model.CatalysisRegimes
    end
@inline _bnc_regimes(model::Bnc) =
    let
        if isnothing(model.BncRegimes)
            error(
                "Model does not contain Bnc regimes. Please run \"match_regimes!($model)\" to compute the Bnc regimes.",
            )
        end
        return model.BncRegimes
    end

@inline _bind_regimes(args...; kwargs...) =
    _bind_regimes(get_binding_network(args...; kwargs...))
@inline _catalysis_regimes(args...; kwargs...) =
    _catalysis_regimes(get_catalysis_network(args...; kwargs...))
@inline _bnc_regimes(args...; kwargs...) =
    _bnc_regimes(get_binding_network(args...; kwargs...))

@inline _bind_regimes_data(args...; kwargs...) =
    _bind_regimes(args...; kwargs...).regimes_data
@inline _catalysis_regimes_data(args...; kwargs...) =
    _catalysis_regimes(args...; kwargs...).regimes_data
...
function n_catalysis_regimes(args...; kwargs...)
    return length(_catalysis_regimes_data(args...; kwargs...))
end
...
function n_bnc_regimes(
    rgms::AbstractArray{<:BncRegime}; feasible::Union{Bool, Nothing}=true, kwargs...
)
    return count(rgm -> isnothing(feasible) || is_feasible(rgm) == feasible, vec(rgms))
end

@inline _bind_regimes_perm_dict(args...; kwargs...) =
...
function get_binding_perm_dict(args...; kwargs...)
    ...
    return _bind_regimes_perm_dict(bn)
end
...
function get_catalysis_perm_dict(args...; kwargs...)
    ...
    return _catalysis_regimes_perm_dict(cn)
end
...
function Base.propertynames(model::CatalysisData, private::Bool=false)
    names = Symbol[fieldnames(typeof(model))..., :d_para]
    return if private
        Tuple(unique(names))
    else
        Tuple(sym for sym in unique(names) if !startswith(String(sym), "_"))
    end
end
#========================================================================================#
...
"""
function ensure_regime_data!(rgm::BindRegime; affine::Bool=true, conditions::Bool=true)
...
function ensure_regime_data!(rgm::CatalysisRegime; affine::Bool=true, conditions::Bool=true)
    _initialize_regime!(rgm)
    return nothing
end

function ensure_regime_data!(rgm::BncRegime; affine::Bool=true, conditions::Bool=true)
    ensure_bnc_regimes!(get_binding_network(rgm))
    return nothing
end

function ensure_regime_data!(rgms::AbstractVector{<:AbstractRegime}; kwargs...)
...
function Base.:(==)(perm::AbstractVector{<:Integer}, rgm::CatalysisRegime)
    return perm == get_catalysis_perm(rgm)
end
function Base.:(==)(rgm::CatalysisRegime, perm::AbstractVector{<:Integer})
    return get_catalysis_perm(rgm) == perm
end
...
function get_binding_index(Bnc::Bnc, idx::T; check::Bool=false) where {T <: Integer}
    check && (ensure_binding_regimes!(Bnc);
...
function get_catalysis_idx(
    model::CatalysisData, idx::T; check::Bool=false
) where {T <: Integer}
    check && (
        ensure_catalysis_regimes!(model); @assert idx >= 1 && idx <= n_regimes(model) "The given catalysis index is out of range."
    )
    return idx
end
...
function get_bnc_idx(rgm::BncRegime)
    return _bnc_linear_index(
...
function is_binding_singular(args...; kwargs...)
    return is_singular(get_binding_regime(args...; kwargs...))
end
...
function is_asymptotic(rgm::BncRegime)
    return (is_asymptotic(rgm.bind_rgm), is_asymptotic(rgm.catalysis_rgm))
end

function is_binding_asymptotic(args...; kwargs...)
    return is_asymptotic(get_binding_regime(args...; kwargs...))
end
is_bind_asymptotic(args...; kwargs...) = is_binding_asymptotic(args...; kwargs...)
function is_catalysis_asymptotic(args...; kwargs...)
    return is_asymptotic(get_catalysis_regime(args...; kwargs...))
end
...
function have_perm(model::AbstractBnc, bind, cat)
    return have_perm(get_binding_network(model), bind) &&
           have_perm(get_catalysis_network(model), cat)
end
#========================================================================================#
...
function _get_filter(;
    ...
    function judge_nullity(x)
        if !isnothing(nullity)
            if nullity isa Integer
                get_nullity(x) == nullity
            else
                nullity[1] <= get_nullity(x) <= nullity[2]
            end
...
function get_bind_regimes(
    rgms::AbstractVector{<:BindRegime}; return_idx::Bool=false, kwargs...
)
...
"""
function get_binding_regimes(
    Bnc::AbstractBnc, rgms::Union{Nothing, AbstractVector}=nothing; kwargs...
)
...
function get_bind_regimes(
    Bnc::AbstractBnc,
    rgms::Union{Nothing, AbstractVector}=nothing;
    return_idx::Bool=false,
    kwargs...,
)
    return if return_idx
        get_binding_indices(Bnc, rgms; kwargs...)
    else
        get_binding_regimes(Bnc, rgms; kwargs...)
    end
end
...
function get_catalysis_regimes(
    Bnc::AbstractBnc, rgms::Union{Nothing, AbstractVector}=nothing; kwargs...
)
    cn = get_catalysis_network(Bnc)
    ensure_catalysis_regimes!(cn)
    rgms =
        isnothing(rgms) ? _catalysis_regimes_data(cn) : get_catalysis_regime.(Ref(cn), rgms)
    return get_catalysis_regimes(rgms; kwargs...)
end

function get_bnc_regimes(
    model::AbstractBnc, rgms::Union{Nothing, AbstractArray}=nothing; kwargs...
)
    bn = get_binding_network(model)
...
function n_bnc_regimes(model::Bnc; feasible::Union{Bool, Nothing}=true, kwargs...)
    return length(get_bnc_regimes(model; feasible=feasible, kwargs...))
end

"""
    get_binding_perms(args...; kwargs...) -> Vector
    get_catalysis_perms(args...; kwargs...) -> Vector
    get_bnc_perms(args...; kwargs...) -> Vector

Return regime permutations for the selected binding, catalysis, or
Binding-Catalysis regimes. Keyword filters are forwarded to the corresponding
`get_*_regimes` function.
"""
get_binding_perms(args...; kwargs...) =
    get_binding_perm.(get_binding_regimes(args...; kwargs...))
get_bind_perms(args...; kwargs...) = get_binding_perms(args...; kwargs...)

"""
    get_binding_indices(args...; kwargs...) -> Vector{Int}
    get_catalysis_indices(args...; kwargs...) -> Vector{Int}
    get_bnc_indices(args...; kwargs...) -> Vector{Int}

Return regime indices for the selected binding, catalysis, or
Binding-Catalysis regimes. Keyword filters are forwarded to the corresponding
`get_*_regimes` function.
"""
get_binding_indices(args...; kwargs...) =
    get_binding_index.(get_binding_regimes(args...; kwargs...))
get_bind_indices(args...; kwargs...) = get_binding_indices(args...; kwargs...)
function get_catalysis_perms(args...; kwargs...)
    return get_catalysis_perm.(get_catalysis_regimes(args...; kwargs...))
end
function get_catalysis_indices(args...; kwargs...)
    return get_catalysis_idx.(get_catalysis_regimes(args...; kwargs...))
end
...
"""
    get_binding_nullities(args...; kwargs...) -> Vector
    get_bnc_nullities(args...; kwargs...) -> Vector

Return nullities for the selected binding or Binding-Catalysis regimes. Keyword
filters are forwarded to the corresponding `get_*_regimes` function.
"""
get_binding_nullities(args...; kwargs...) =
    get_binding_nullity.(get_binding_regimes(args...; kwargs...))
get_bind_nullities(args...; kwargs...) = get_binding_nullities(args...; kwargs...)
function get_bnc_nullities(args...; kwargs...)
    return get_bnc_nullity.(get_bnc_regimes(args...; kwargs...))
end
...
get_affine_xk2qKk(rgm::BindRegime) =
    let
...
get_affine_x2qcat(rgm::BindRegime) =
    let
...
get_affine_xk2f(rgm::CatalysisRegime) =
    let
...
function get_affine_qK2x(rgm::BindRegime)
    let
...
get_affine_qKk2xk(rgm::BindRegime) =
    let
...
get_affine_qKk2v(rgm::BncRegime) =
    let
...
@inline function _maybe_remove_h_redundancy_pair(
    C::AbstractMatrix{<:Real}, C0::AbstractVector; remove_h_redundancy::Bool=false
)
    C_new, C0_new, _ = _maybe_remove_h_redundancy(
        C, C0, 0; remove_h_redundancy=remove_h_redundancy
    )
    return C_new, C0_new
end
...
function get_C_C0_x(rgm::BindRegime; remove_h_redundancy::Bool=false)
    ...
    return _maybe_remove_h_redundancy_pair(
        rgm.C_x, rgm.C0_x; remove_h_redundancy=remove_h_redundancy
    )
end
function get_C_C0_xk(rgm::BindRegime; remove_h_redundancy::Bool=false)
    let
        n_k = get_catalysis_network(rgm).n_k
        C_x, C0_x = get_C_C0_x(rgm)
        C_xk = hcat(C_x, spzeros(eltype(C_x), size(C_x, 1), n_k))
        return _maybe_remove_h_redundancy_pair(
            C_xk, C0_x; remove_h_redundancy=remove_h_redundancy
        )
    end
end
function get_C_C0_v(rgm::CatalysisRegime; remove_h_redundancy::Bool=false)
    get_catalysis_regime(rgm)
    return _maybe_remove_h_redundancy_pair(
        rgm.C, rgm.C0; remove_h_redundancy=remove_h_redundancy
    )
end
...
function get_C_C0_xk(rgm::CatalysisRegime; remove_h_redundancy::Bool=false)
    let
        C, C0 = get_C_C0_v(rgm)
        z, z0 = get_affine_xk2v(rgm)
        C_xk = C * z
        C0_xk = C * z0 + C0
        return _maybe_remove_h_redundancy_pair(
            C_xk, C0_xk; remove_h_redundancy=remove_h_redundancy
        )
    end
end

# This is the steady-state.
function get_C_C0_nullity_xk(rgm::CatalysisRegime; remove_h_redundancy::Bool=false)
    let
...
function get_C_C0_xk(rgm::BncRegime; remove_h_redundancy::Bool=false)
    let
...
function get_C_C0_nullity_xk(
    rgm::BncRegime, kind::Symbol=:combined; remove_h_redundancy::Bool=false
)
    if kind === :binding
        C, C0 = get_C_C0_xk(get_binding_regime(rgm))
        return _maybe_remove_h_redundancy(C, C0, 0; remove_h_redundancy=remove_h_redundancy)
    elseif kind === :catalysis
        return get_C_C0_nullity_xk(
            get_catalysis_regime(rgm); remove_h_redundancy=remove_h_redundancy
        )
...
function get_C_C0_nullity_qKk(rgm::BindRegime; remove_h_redundancy::Bool=false)
    let
        ...
        return _maybe_remove_h_redundancy(
            C, C0_qK, nullity; remove_h_redundancy=remove_h_redundancy
        )
    end
end
function get_C_C0_nullity_qKk(
    ...
    if kind === :binding
        return get_C_C0_nullity_qKk(
            get_binding_regime(rgm); remove_h_redundancy=remove_h_redundancy
        )
    elseif kind === :catalysis
        return _maybe_remove_h_redundancy(
            rgm.C_qKk_cat,
...
get_affine_x2k̃(rgm::CatalysisRegime) =
    let
        get_catalysis_regime(rgm)
...
#  ̃p := -P\logk
get_affine_k2k̃(rgm::CatalysisRegime) =
    let
        get_catalysis_regime(rgm)
...
function get_affine_x2Kk̃(rgm::BncRegime)
    let
        bind_rgm = get_binding_regime(rgm.bind_rgm; inv_info=false)
...
function get_affine_x2wKk̃(rgm::BncRegime)
    let
        bind_rgm = get_binding_regime(rgm.bind_rgm; inv_info=false)
        cat_rgm = get_catalysis_regime(rgm.catalysis_rgm)
...
get_affine_xk2wKk̃k(rgm::BncRegime) =
    let
...
function get_affine_wKk2wKk̃k(rgm::BncRegime)
    let
...
get_affine_wKk2wKk̃(rgm::BncRegime) =
    let
        d_w = get_catalysis_network(rgm).d_w
        r = get_binding_network(rgm).r
        blk1 = spdiagm(0 => ones(Int, d_w))
...
get_affine_wKk̃2x(rgm::BncRegime) =
    let
        H = rgm.H_inner
        H0 = rgm.H0_inner
        return H, H0
...
get_affine_wKk̃k2xk(rgm::BncRegime) =
    let
        H = rgm.H_inner
        H0 = rgm.H0_inner
        n_k = get_catalysis_network(rgm).n_k
...
function get_affine_wKk2x(rgm::BncRegime)
    let
...
function get_affine_wKk2xk(rgm::BncRegime)
    let
...
get_affine_wKk2v(rgm::BncRegime) =
    let
        H, H0 = get_affine_wKk2x(rgm)
        n_k = get_catalysis_network(rgm).n_k
        H_cat = blockdiag(H, spdiagm(0 => ones(Int, n_k)))
...
function get_affine_wKk2qcat(rgm::BncRegime)
    let
        rgm.nlt == 0 || error(
            "The reduced steady-state system is singular, so q_cat has no affine expression in (w, K, k).",
        )
        H, H0 = get_affine_wKk2x(rgm)
        r_v = get_catalysis_network(rgm).r_v
...
function get_P_P0_x(args...; kwargs...)
    return get_P_P0_x(get_binding_regime(args...; inv_info=false, kwargs...))
end
...
function get_P_P0_pos_neg(args...; kwargs...)
    return get_P_P0_pos_neg(get_catalysis_regime(args...; kwargs...))
end
...
function get_M_M0(args...; kwargs...)
    return get_M_M0(get_binding_regime(args...; inv_info=false, kwargs...))
end
...
function get_C_C0_x(args...; remove_h_redundancy::Bool=false, kwargs...)
    return get_C_C0_x(
...
function get_C_C0_xk(args...; remove_h_redundancy::Bool=false, kwargs...)
    return get_C_C0_xk(
        get_binding_regime(args...; inv_info=false, kwargs...);
        remove_h_redundancy=remove_h_redundancy,
    )
...
function get_C_C0_nullity_qK(args...; remove_h_redundancy::Bool=false, kwargs...)
    return get_C_C0_nullity_qK(
...
get_C_C0(args...; kwargs...) = (x -> (x[1], x[2]))(get_C_C0_nullity(args...; kwargs...))
