...
function show_condition_poly(poly::Polyhedron; remove_h_redundancy::Bool=false, kwargs...)
    return show_condition_poly(
        get_C_C0_nullity(poly)...; remove_h_redundancy=remove_h_redundancy, kwargs...
    )
end
function show_condition_poly(C_qK::AbstractVector{<:Real}, C0_qK::Real, args...; kwargs...)
    return show_condition_poly(C_qK', [C0_qK], args...; kwargs...)[1]
end
...
function show_expression_mapping(
    ...
    if log_space
        ...
        return if asymptotic
            y .~ handle_log_weighted_sum(C, x)
        else
            y .~ handle_log_weighted_sum(C, x, C0)
        end
    end
end

function show_expression_mapping(C::AbstractVector{<:Real}, C0::Real, args...; kwargs...)
    return show_expression_mapping(C', [C0], args...; kwargs...)[1]
end

@inline _render_condition_from(data::Tuple, syms; kwargs...) =
    show_condition_poly(data...; syms=syms, kwargs...)
@inline _render_expression_from(data::Tuple, y, x; kwargs...) =
    show_expression_mapping(data..., y, x; kwargs...)
...
function handle_log_weighted_sum(
    A::AbstractMatrix{<:Real}, x, b::Union{Nothing, AbstractVector{<:Real}}=nothing
)::Vector{Num}
    rows = size(A, 1)
    rst = Vector{Num}(undef, rows)
    b = isnothing(b) ? zeros(Int, rows) : b
    for i in 1:rows
        rst[i] = (u -> u * _exp10_factor(b[i]))(prod(x .^ A[i, :]))
    end
    return rst
end
