...
_calc_c_c0(hp::RegimeHyperplane, dir::Int8) = let
    if dir > 0
...
get_hp_key(c::SparseVector{<:Rational}, c0) = HyperplaneKey(Tuple(c.nzind), Tuple(c.nzval), c0)
...
function _canonicalize_halfspace(
...
_canonicalize_hyperplane(args...;kwargs...) = _canonicalize_halfspace(args...; kwargs...)[1]
...
add_hyperplane!(db::RegimeToHyperplanePool, hp::RegimeHyperplane; canonicalize::Bool=false) = add_hyperplane!(db, hp.change_dir_qK, hp.intersect_qK; canonicalize=canonicalize)




# function add_hyperplane!(db::RegimeToHyperplanePool, c::SparseVector{<:Rational}, c0::ExactLogExpr; canonicalize::Bool=true)
function add_halfspace!(db::RegimeToHyperplanePool, c::SparseVector{<:Rational}, c0::ExactLogExpr, dir::Int8; canonicalize::Bool=true)
